...
import { EASE, WA_LINK } from "@/lib/constants";
...
function GlobeSkeleton() {
  return (
    <div
      className="absolute inset-0 animate-pulse bg-[radial-gradient(ellipse_at_center,rgba(200,151,62,0.08)_0%,transparent_55%)]"
      aria-hidden="true"
    />
  );
}
...
export function Hero({ ready = false }: HeroProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_0%,var(--color-bg)_72%)]"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-b from-bg/40 via-transparent to-bg/90"
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-7xl flex-col items-center justify-center px-6 pt-20 pb-24 text-center md:px-12 md:pt-24">
        ...
        <h1
          ref={headlineRef}
          className={cn(
            "mt-6 max-w-5xl font-display font-bold tracking-[-0.03em] text-ivory",
            "text-[clamp(2.5rem,8vw,7.5rem)] leading-[1.05]",
          )}
        >
        ...
        <p
          ref={subRef}
          className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-sage md:text-lg"
        >
        ...
        <div
          ref={badgesRef}
          className="mt-12 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 font-mono text-[11px] uppercase tracking-[0.08em] text-ivory/70"
        >

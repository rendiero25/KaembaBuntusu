      ...
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        <div data-section-item>
          <h2 className="max-w-3xl font-display text-[clamp(2rem,4.5vw,3.5rem)] font-bold leading-[1.05] text-ivory">
            Four export-grade commodities from Sulawesi.
          </h2>
          <p className="mt-4 max-w-xl text-base leading-relaxed text-sage">
            Coconut, copra, cloves, and pepper. Every product graded and
          ...
          {PRODUCTS.map((product, index) => (
            ...
            >
              <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden rounded-t-sm">
                ...
                />
                <span className="absolute bottom-3 left-4 z-[1] font-mono text-[11px] uppercase text-gold">
                  {product.number}
                </span>
              </div>
              <div className="flex flex-1 flex-col p-5 md:p-6">
                <h3 className="font-heading text-lg font-semibold text-ivory transition-colors group-hover:text-gold md:text-xl">
                  {product.name}
                </h3>
                <p className="mt-2 line-clamp-2 flex-1 text-sm leading-relaxed text-sage">
                  {product.tagline}
                </p>
                <span className="mt-4 inline-flex items-center gap-2 font-mono text-[10px] uppercase text-gold/80 transition-colors group-hover:text-gold">
                  View specs
                  <span
